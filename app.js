  ...
  try {
    ...
    if (!existingLotDate) {
      ...
      const playzoneData = {
        ...
        playnumbers: createPlaynumbersArray(location.maximumNumber),
      };
      await Playzone.create(playzoneData);

      // Create PartnerPerformance if not exists
      const existingPerformance = await PartnerPerformance.findOne({
        lotlocation: location._id,
        lottime: time._id,
        lotdate: newLotDate._id,
      });
      ...
      if (!existingPerformance) {
        await PartnerPerformance.create({
          lotlocation: location._id,
          lottime: time._id,
          lotdate: newLotDate._id,
          performances: [],
